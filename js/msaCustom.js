...
function pluckInOut (data, from , to) {
    if (from == to) return;
...
function testMsa (target, nSeq, nCol) {
    ...
    console.dir(msa);
    msaObj = newMsa({data : msa, target : target, seqName : seqName});
    msaObj.draw();

}

function newMsa (options) {
    if (!options.data) {
    ...
    for (var i = 0; i < nSeq ; i++) {
        ...
        for (var j = 0; j < nCol ; j++) {
            ...
            if (options.data[i][j] !== '-') {
                data[i][j]['location'] = options.locations[i][data[i][j]['pos']];
                offset++;
            } else {
    ...
    var topSliderHeight = 20, columnLabelHeight = 15;
    var topOffset = topSliderHeight + columnLabelHeight;

    var leftSliderWidth = 20, rowLabelWidth = 95;
    ...
    return {
            ...
            topOffset : topOffset,
            msaViewPoint : { w : nColDisplayed * rw, h :nRowDisplayed * rh },
            ...
            yBufferSize : 5,
            boundaries : {
                x : { min : 0, max : nColDisplayed / 2 },
                y : { min : 0, max : nRowDisplayed / 2 }
            },
            topSlider : { h : topSliderHeight, w : topSliderHeight}, // circle for now
            columnLabelHeight : columnLabelHeight,
            leftSlider : { h : leftSliderWidth, w :leftSliderWidth}, // circle for now
    ...
    // Precompute sin and tan waves
                ...
                if (datum.sse == "helix") {
                    d3.select(cell).append("g")
                        ...
                        .selectAll("path")
                            ...
                            .attr("class", "wave")
                            .attr("d", d3.svg.line()
                                .x(function(d, i) { return self.trigo.x(i * self.trigo.INTERVAL) - self.trigo.x(0) })
                                .y(function(d) { return self.trigo.y(d) }))
                            .style("stroke", "black")
            ...
            toggleCellColor : function(optColor) {
                var regCode = {"default" : true , "location" : true};
                if (!optColor) {
            ...
            _swapRow : function (from, to) {// maybe directly supply the responsible DOM object
                pluckInOut({matrix : this.data}, from, to);
                pluckInOut({list : this.names}, from, to);
                this._paint();
                ...
                if ('name' in data) { // Not working properly
                    for (var i = 0; i < this.names.length; i++) {
                        ...
                        if (this.names[i] === data.name) {
                            ...
                            var dy = i > this.nRow - this.nRowDisplayed ? this.nRow - this.nRowDisplayed
                                                                        : i ;
                            ...
                            this.scroll({"dy" : dy, 'Ry' : Ry, 'Dy' : Dy});



                            this.svg.selectAll('.handleV').attr("transform", function (){
                                   var trCoor = getTranslationCoordinates(d3.select(this).attr("transform"));
            ...
            $(self.target + ' div.msaHeader').append('<div class="typeAheadWrap">'
                + '<input class="typeahead" type="text" placeholder="Sequence name"></div>'
                );
            ...
            $(self.target + ' .typeAheadWrap .typeahead').on('typeahead:selected', function (){
                self._moveTo({name : this.value});
            });
            $(self.target + " .fasearch").on('click', function(){self._moveTo(); }/*self._moveTop()*/);
            ...
            this.svg.selectAll('defs').append('clipPath').attr("id", "seqNameAreaClip")
                ...
                self._paint({xmin : 0, xmax : self.nColDisplayed, ymin : 0, ymax : self.nRowDisplayed});
                ...
                if (self.draggable)
                    $(self.target).drags({handle : "div.msaHeader"});
                ...
                _paint : function (coor) {
                    ...
                    var data = crop(self.data, {xmin : xmin, xmax : xmax, ymin : ymin, ymax : ymax});
                  ...
                  //  console.dir(data);
                   ...
                   // grp.exit().transition().delay("250").remove();
                    ...
                    var cellGrp = grp.selectAll('.msaCell')
                        .data(function(d) { return d; }, function (d, i){ return 'c' + i;});
                    ...
                    grp.selectAll('.msaCell')
                        .append("text")
                        .attr("x", function(d) { return self.rw/2 - 5; })
                        .attr("y", self.rh / 2)
                        .attr("dy", ".15em")
                        .attr('font-size', function (d){ return (self.rh / 4) < 10 ? 10 + "px" : (self.rh / 4) + "px";})
                        .style('fill', "black")
                    ...
                    if (self.locations){
                        var ll = grp.selectAll('.msaCell').select(function(d){ if(d.location) return this; return null});
                            ll.append("path")
                            .attr("d", d3.svg.symbol()
                                        ...
                                        .size(function(){ return self.rh * 0.6;})
                            )
                ...
                _getSeqNameAreaViewPoint : function () { // return top and bot y coordinates of leftPanel
                    ...
                    var trans = getTranslationCoordinates(string);

                    return {min : -1 * trans[1] + this.topOffset,
                ...
                _drawSeqNameArea : function (resetBool) {
                    ...
                    */
                    ...
                    function dragstarted(d) {
                       ...
                       //d3.event.sourceEvent.stopPropagation();
                        ...
                        d3.select(this).transition().attr("class", function (){
                            return d3.select(this).attr("class") + " dragging";}).delay(250);
                    ...
                    function dragged(d) {
                        var currCoor = getTranslationCoordinates(d3.select(this).attr("transform"));
                        ...
                        d3obj = this;
                        if (newY == vp.min) {
                            self.scroll({increment : 'top'}).done(function(top){
                    ...
                    function dragended(d) {
                        ...
                        yEnd = getTranslationCoordinates(d3.select(this).attr("transform"))[1];
                        yEnd -= self.topOffset;
                        /*console.log(iStart);
                        console.log(yEnd);*/
                    ...
                    seqNameArea.selectAll('.seqName')
                        ...
                        .attr('class', function (d, i){ return 'seqName s' + i;})
                        .attr('transform', function(d, i) {
                    ...
                    seqNameArea.selectAll('.seqName').append('foreignObject')
                        ...
                        .html(function(d) {
                            if (self.seqBind) {
                                return '<div class="nameCell"><i class="fa fa-search pull-left"></i>' + d
                        ...
                        if (self.seqBind) {
                            d3.selectAll('.msaSeqNameArea i.fa-search').on('mousedown', function(d,i){
                                ...
                                self.data[i].forEach(function (elem){
                                    sequence.push({ "letter" : elem.letter, "pos" : elem.pos});
                                });
                ...
                _drawConsBand : function () {
                    ...
                    bars.selectAll('.consBar').append('rect')
                        .attr('height', function (d){ return hScale(d);}).attr('width', self.rw)
                        .style('fill', 'steelblue')
                        .attr("y", function (d){ return h - hScale(d);});
                        //.attr("y", 0);
                ...
                _drawColumnLabels : function () {
                    ...
                    var numeral = group.selectAll('.colLabelCell').filter(function(d){return d.shown === "numeral" ? true : false;})
                        .append("text")
                        .attr('class', "numeral");
                    var dotted = group.selectAll('.colLabelCell').filter(function(d){ return d.shown === "dotted" ? true : false;})
                        .append('circle')
                        .attr('class', "dotted");
                ...
                _resetHoriSlider : function () { // Left element on change, update is made on self.leftOffset value
                    ...
                    var currCoor = getTranslationCoordinates(d3.select('g.sliderH').attr("transform"));
                    ...
                    var hCurrCoor = getTranslationCoordinates(d3.select('g.sliderH circle.handleH').attr("transform"));
                    d3.selectAll("g.sliderH circle.handleH");

                    /*var handle = d3.selectAll("g.sliderH circle.handleH");
                    var oldX = getTranslationCoordinates(handle.attr("transform"))[0];*/
...
// HoverIntent to smooth things
                ...
                //    console.log( "x = " + x +"\nnw = " + nw + "\nleftOffset = " + self.leftOffset);
                    ...
                    d3.selectAll(".nudgeable").attr('transform', function (){
                        if (d3.select(this).classed("sliderV")){
                            var currCoor = getTranslationCoordinates(d3.select('g.sliderV').attr("transform"));
                            return 'translate(' + x + ', ' + currCoor[1] + ')';
                        }
                        return 'translate(' + (self.leftOffset  - self.rowLabelWidthMin  - self.leftSlider.w - 5) + ' , 0)';
                    });
                    self._refreshClip({'seqNameArea' : self.rowLabelWidth});
            ...
            scroll : function(opt){
              /*  console.log("scroll options");
                ...
                if ('increment' in opt) { //
                    var c = opt.increment === 'top' ?  d3.selectAll('g.seqName').first()
                                                    :  d3.selectAll('g.seqName').last();
                    if (opt.increment === 'top') {

                        var string = this.svg.select("g.msaSeqNameArea").attr("transform");
                        var trans = getTranslationCoordinates(string);
                        var newY = trans[1] + this.rh > 0
                                    ? 0
                                    : trans[1] + this.rh;
                        ...
                        this._paint({ymin : ymin, ymax : ymax});
                ...
                if ('dx' in opt) {
                    ...
                    this._paint({ xmin : Rx - Dx , xmax : Rx + this.nColDisplayed - Dx });
                    // perform translation
                    var dx = opt.dx < 0 ? 0 : Math.round(opt.dx);
                    dx = (-1) * dx * rw;
                    var translitteralX = this.svg.select(".zMsaLayer").attr("transform");
                    var trCoor = getTranslationCoordinates(translitteralX);
                ...
                if('dy' in opt){
                    ...
                    this._paint({ ymin : Ry - Dy , ymax : Ry + this.nRowDisplayed - Dy });

                    var dy = opt.dy < 0 ? 0 : Math.round(opt.dy);
                    dy = (-1) * dy * rh;

                    var translitteralY = this.svg.select(".zMsaLayer").attr("transform");
                    var trCoor = getTranslationCoordinates(translitteralY);
            ...
            locSpecs : {'I' : "red", 'E' : "blue", 'M' : 'yellow'},
            aaSpecs : { 'A' : { color : 'yellow', charge : '0', polar : false},
                        'C' : { color : 'yellow', charge : '0', polar : true },
                        'D' : { color : 'red', charge : '-1', polar : true},
                        'E' : { color : 'red', charge : '-1', polar : true},
                        'F' : { color : 'yellow', charge : '0', polar : false},
                        'G' : { color : 'grey', charge : '0', polar : false},
                        'H' : { color : 'blue', charge : '+1', polar : true},
                        'I' : { color : 'yellow', charge : '0', polar : false},
                        'K' : { color : 'blue', charge : '+1', polar : true},
                        'L' : { color : 'yellow', charge : '0', polar : false},
                        'M' : { color : 'yellow', charge : '0', polar : false},
                        'N' : { color : 'orange', charge : '0', polar : true},
                        'P' : { color : 'yellow', charge : '0', polar : false},
                        'Q' : { color : 'orange', charge : '0', polar : true},
                        'R' : { color : 'blue', charge : '+1', polar : true},
                        'S' : { color : 'orange', charge : '0', polar : true},
                        'T' : { color : 'orange', charge : '0', polar : true},
                        'V' : { color : 'yellow', charge : '0', polar : false},
                        'W' : { color : 'yellow', charge : '0', polar : false},
                        'Y' : { color : 'yellow', charge : '0', polar : true},
                        '-' : { color : 'grey', charge : 'na', polar : 'na'}
            }
    };
